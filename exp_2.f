...
!                Rbin=Rbin+real(sumR)/real(n)
                ...
                sumR=0
                do m=1,L
                  sumR=sumR+sigma(m)*(m-(L/2))**2
                enddo
                Rbin=real(sumR)/real(n)
              endif
...
111         if ((t.lt.real(tmax)).and.((t.ne.h))) then !si t<tmax se ha activado la CC, si t/=h no se actualizaron  mean_
                mean_p(i)=mean_p(i)+pbin/(t-1.5**(i-1))
                mean_n(i)=mean_n(i)+real(nbin)/(t-1.5**(i-1))
                mean_R(i)=mean_R(i)+Rbin/(t-1.5**(i-1))
            endif
